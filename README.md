<div align="center"><center>

<img alt="Icon" width=100 src="https://raw.githubusercontent.com/TerminalMC/EffectTimerPlus/HEAD/common/src/main/resources/assets/effecttimerplus/icon.png">

## EffectTimerPlus

Adds a potency indicator and countdown overlay to status effect icons.

An update of [Status Effect Timer](https://modrinth.com/project/T9FDHbY5) with a focus on customizability.

[![Environment](https://img.shields.io/badge/Environment-Client-blue?logo=data:image/png;base64,)]()
[![Latest Minecraft](https://img.shields.io/modrinth/game-versions/JIUF2Wb5?label=Latest%20Minecraft&color=%2300AF5C&logo=data:image/png;base64,)](https://modrinth.com/project/JIUF2Wb5/versions)

[![Loader](https://img.shields.io/badge/Available%20for-Fabric-dbd0b4?logo=data:image/png;base64,)](https://fabricmc.net/)
[![Loader](https://img.shields.io/badge/Available%20for-NeoForge-f16436?logo=data:image/png;base64,)](https://neoforged.net/)

[![Available on Modrinth](https://img.shields.io/modrinth/dt/JIUF2Wb5?label=Available%20on%20Modrinth&logo=modrinth&logoColor=%2300AF5C)](https://modrinth.com/project/JIUF2Wb5)
[![Available on CurseForge](https://img.shields.io/curseforge/dt/1049882?label=Available%20on%20CurseForge&logo=curseforge)](https://curseforge.com/minecraft/mc-mods/effecttimerplus)
[![Available on GitHub](https://img.shields.io/github/downloads/TerminalMC/EffectTimerPlus/total?label=Available%20on%20GitHub&logo=github&logoColor=white)](https://github.com/TerminalMC/EffectTimerPlus)

</center></div>

### Features

- Fully adjustable text color and opacity
- Optional text shadow
- Optional and adjustable text background
- 8 options for text position
- Optional low-time warning color and flash
- Option to disable either indicator separately
- Option to hide timer on ambient (beacon) effects
- Scaling option to change the size of effect icons
- Scaling option to change the text size relative to the effect icon

<img alt="Showcase" src="https://raw.githubusercontent.com/TerminalMC/EffectTimerPlus/HEAD/assets/images/showcase/effects.png" width="500">

### Dependencies

Fabric: [Fabric API](https://modrinth.com/project/P7dR8mSH/), [ModMenu](https://modrinth.com/project/mOgUt4GM), [YACL](https://modrinth.com/project/1eAoo2KR)

NeoForge: [YACL](https://modrinth.com/project/1eAoo2KR)

### Compatibility

Sometimes the scaling option can cause problems for other mods. If you encounter issues, please report on Discord or GitHub.

### Credits

EffectTimerPlus is based on [Status Effect Timer](https://github.com/magicus/statuseffecttimer) by magicus and includes 
code reused under license.

### Contact

[![Discord Server](https://img.shields.io/discord/1103153365216669797?logo=discord&label=Discord%20Server&color=%235865F2)](https://discord.terminalmc.dev)

[![GitHub Issues](https://img.shields.io/github/issues/TerminalMC/EffectTimerPlus?logo=github&label=GitHub%20Issues)](https://github.com/TerminalMC/EffectTimerPlus/issues)

[![License](https://img.shields.io/github/license/TerminalMC/EffectTimerPlus?label=License&logo=github&logoColor=white)](https://github.com/TerminalMC/EffectTimerPlus/blob/HEAD/LICENSE.txt)
